...
############################# get units main ##########################
def get_units_main(mosq,obj,msg):
    #print "get_units......."
...
# UnitId | IpMacid      | IpName    | OpMacid      | OpName    | PMacId       | PName      | Fan  | Status | Temp | setTemp | Voltage | Power | Current | Voltage | Power | Current | UnitId | UnitId |
       ...
       if history_all == True:
          '''
          sqlq="SELECT * FROM (SELECT IFNULL( (select count(*)as count  from AC_Input_Units where Temp between 23 and 25 and ChangeBy='jts_admin' GROUP BY CityId) ,'0') as good) A, (SELECT IFNULL( (select count(*)as count  from AC_Input_Units where Temp between 25 and 27 and ChangeBy='jts_admin' GROUP BY CityId) ,'0') as warning) B, (SELECT IFNULL( (select count(*)as count  from AC_Input_Units where Temp between 20 and 22 and ChangeBy='jts_admin' GROUP BY CityId) ,'0') as problem) C, (SELECT IFNULL( (select count(*)as count from AC_Input_Units where Temp=50 and ChangeBy='jts_admin' GROUP BY CityId) ,'0') as main) D,
(SELECT IFNULL( (select round(Avg(AC_Input_History.Temp))as temp from AC_Input_History,AC_Input_Units where AC_Input_History.IpId=AC_Input_Units.IpId and AC_Input_Units.ChangeBy='jts_admin' and DATE_ADD(AC_Input_History.ChangeDate, INTERVAL 10 MINUTE) >= NOW() group by AC_Input_Units.CityId) ,'0') as avgTemp) E;"
          '''         
          #sqlq = "select count(DISTINCT UnitId)  from Oyo_Unit_History where Temp between 23 and 25 GROUP BY CityId"
          sqlq = "select count(*)as count  from AC_Input_Units where Temp between 23 and 25 and ChangeBy='%s' GROUP BY CityId" %(username)
          cursor.execute(sqlq)
          ...
          if len(get_temp) > 0 or len(get_power) > 0 or len(get_voltage) > 0:
             ...
             if len(get_voltage) > 0:
                ...
                for rec in get_voltage:
                    ...
                    stdate = datetime.strftime(rec[1], '%Y-%m-%d %H:%M')
                    if(counter == 1):
                       output += '{"motion":"%s","changedate":"%s"}' %(int(rec[0]) ,stdate)
                    else:
                       output += ',\n {"motion":"%s","changedate":"%s"}' %(int(rec[0]) ,stdate)
          ...
          if(len(get_power_rec) > 0):
             ...
             for i,rec in enumerate(get_power_rec):
                 if i in range(0,len(get_saved_units_rec)):
                    ...
                    #if get_saved_units_rec[i][1] in get_power_rec[i]:
                    #   date1=get_saved_units_rec[i][1]
                    #else:
                    #   date1=get_power_rec[i][1]
                 #print date1
                 counter += 1
                 if(counter == 1):
